...
SECRET_PATTERNS = [
    ...
    ("Generic API Key", r'(?i)(api[_-]?key|apikey)\s*[=:]\s*["\']?([A-Za-z0-9_\-]{20,})', Severity.HIGH),
    ...
    ("Slack Webhook", r'https://hooks\.slack\.com/services/T[A-Z0-9]{8}/B[A-Z0-9]{8}/[A-Za-z0-9]{24}', Severity.HIGH),
    ...
    ("Generic Token", r'(?i)(token|bearer)\s*[=:]\s*["\']?([A-Za-z0-9_\-\.]{20,})', Severity.MEDIUM),
    ("Connection String", r'(?i)(mongodb|postgres|mysql|redis|amqp):\/\/[^\s"\']+:[^\s"\']+@', Severity.HIGH),
    ("Base64 Encoded Secret", r'(?i)(password|secret|key|token)\s*[=:]\s*["\']?[A-Za-z0-9+/]{40,}={0,2}["\']?', Severity.MEDIUM),
]

# File extensions to skip
SKIP_EXTENSIONS = {'.png', '.jpg', '.jpeg', '.gif', '.svg', '.ico', '.woff', '.woff2', '.ttf', '.eot',
                   '.mp3', '.mp4', '.zip', '.tar', '.gz', '.pdf', '.exe', '.dll', '.so', '.pyc',
                   '.lock', '.min.js', '.min.css'}

# Directories to skip
SKIP_DIRS = {'node_modules', '.git', 'venv', '__pycache__', '.next', 'dist', 'build', '.tox', '.mypy_cache'}
...
class SecretsScanner(BaseScanner):
    ...
    def _is_likely_false_positive(self, line: str, file_path: str) -> bool:
        ...
        fp_indicators = ['example', 'placeholder', 'your_', 'xxx', 'changeme', 'todo',
                         'fake', 'dummy', 'test_key', 'sample', '<your', '{your']
    ...
    async def _scan_git_history(self, target: Path, scan_id: str) -> list[Finding]:
        ...
        try:
            proc = await asyncio.create_subprocess_exec(
                'git', '-C', str(git_root), 'log', '--diff-filter=A', '-p', '--all',
                '-n', '50', '--', '*.env', '*.key', '*.pem', '*.p12', '*.pfx',
                ...
                try:
                    ...
                    for line_num, line in enumerate(content.split('\n'), 1):
                        ...
                        if '=' in line:
                            ...
                            if value and len(value) > 5 and not self._is_likely_false_positive(line, str(env_file)):
                                key_lower = key.lower()
                                if any(s in key_lower for s in ['secret', 'password', 'key', 'token', 'auth', 'credential']):
                                    findings.append(Finding(
                                        scan_id=scan_id,
                                        scanner=self.name,
                                        scan_type=self.scan_type,
