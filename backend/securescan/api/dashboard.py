...
from ..database import get_findings, get_scans
...
async def install_scanner(scanner_name: str):
    ...
    if scanner_name not in INSTALLABLE_SCANNERS:
        raise HTTPException(
            status_code=400,
            detail=f"Scanner '{scanner_name}' cannot be auto-installed. Install it manually.",
        )
...
async def aggregate_stats():
    ...
    avg_risk = sum(s.risk_score for s in completed) / len(completed) if completed else 0.0
    return {
...
async def trends(days: int = Query(30, ge=1, le=365)):
    ...
    completed = [s for s in scans if s.status.value == "completed" and s.completed_at is not None]
