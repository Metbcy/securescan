...
class BaselineScanner(BaseScanner):
        ...
        if not firewall_active:
            try:
                ...
                lines = [
                    ...
                    if ln and not ln.startswith("Chain") and not ln.startswith("target")
                ]
                if lines:
                    firewall_active = True
            except (FileNotFoundError, asyncio.TimeoutError, Exception):
        ...
        for key, value in os.environ.items():
            ...
            for pattern in secret_patterns:
                if pattern in key_upper:
                    ...
                    masked = (
                        value[:2] + "*" * min(len(value) - 4, 20) + value[-2:]
                        if len(value) > 4
                        else "****"
                    )
                    findings.append(
                        ...
                        if st.st_mode & stat.S_IWOTH:
                            findings.append(
                                Finding(
                                    ...
                                    title="Root cron job executes world-writable script",
                ...
                if addr_part in ("0.0.0.0", "*", "::"):
                    findings.append(
                        Finding(
                            ...
                            remediation="If this service should not be publicly accessible, bind it to 127.0.0.1 or restrict access with a firewall.",
                        )
                    )
    ...
    async def _check_package_updates(self, scan_id: str) -> list[Finding]:
        ...
        try:
            ...
            output = stdout.decode()
            lines = [ln for ln in output.strip().split("\n") if ln and "Listing..." not in ln]
            security_updates = [ln for ln in lines if "security" in ln.lower()]
            if security_updates:
                findings.append(
                    Finding(
                        scan_id=scan_id,
